...
final class BrowserChromeAppearanceTests: XCTestCase {
    ...
    func testSelectedTabMatchesHeaderAndInactiveTabsStayGray() {
        for sample in [ChromeRGB.white, ChromeRGB(red: 0, green: 0, blue: 0), ChromeRGB(red: 0.02, green: 0.03, blue: 0.12)] {
            ...
            XCTAssertEqual(palette.selectedTabOpacity, 1)
            XCTAssertEqual(BrowserChromePalette(sample: sample, collapsed: true).selectedTabOpacity, 1)
            XCTAssertEqual(palette.inactiveTab, .tabGray)
        }
        ...
        XCTAssertEqual(halfway.selectedTabOpacity, 1)
