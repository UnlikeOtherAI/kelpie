...
class AccountBookmarks(
    ...
    sealed class Mutation {
        data class Add(val bookmark: Bookmark) : Mutation()
        data class Remove(val id: String) : Mutation()
        data object Clear : Mutation()
    }
    private var active = true
    private var pending: Deferred<Unit>? = null

    fun invalidate() { active = false; pending?.cancel() }
    fun enqueue(mutation: Mutation? = null): Deferred<Unit> {
        ...
        val operation = scope.async {
            ...
            try {
                ...
                if (active) { publish(result); status(false, null) }
    ...
    suspend fun flush() { pending?.await() }
    ...
    companion object {
        ...
        fun decode(raw: JsonArray): List<Bookmark> = raw.mapNotNull { value ->
            val item = value as? JsonObject ?: return@mapNotNull null
            val url = (item["url"] as? JsonPrimitive)?.contentOrNull?.takeIf { it.isNotBlank() } ?: return@mapNotNull null
            Bookmark(id = identifier(item), title = (item["title"] as? JsonPrimitive)?.contentOrNull
                ?: (item["name"] as? JsonPrimitive)?.contentOrNull ?: url, url = url,
                createdAt = date(item["createdAt"] as? JsonPrimitive ?: item["created_at"] as? JsonPrimitive))
        }.distinctBy { it.id }
        fun apply(raw: JsonArray, mutation: Mutation): JsonArray = when (mutation) {
            ...
            is Mutation.Add -> {
                if (decode(raw).any { it.url == mutation.bookmark.url }) raw else JsonArray(raw + buildJsonObject {
                    put("id", mutation.bookmark.id); put("title", mutation.bookmark.title)
                    put("url", mutation.bookmark.url); put("createdAt", mutation.bookmark.createdAt)
                })
            }
        }
        private fun date(value: JsonPrimitive?): String {
            ...
            return runCatching { java.time.Instant.ofEpochMilli(((seconds + 978307200) * 1000).toLong()).toString() }.getOrDefault("")
        }
