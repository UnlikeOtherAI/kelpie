...
class UOATransport {
    data class Response(val data: ByteArray, val version: String?) {
        fun json(): JsonObject = Json.parseToJsonElement(data.toString(Charsets.UTF_8)).jsonObject
    }
    class Failure(val status: Int) : Exception(when (status) {
    ...
    suspend fun request(path: String, method: String = "GET", token: String? = null,
        body: String? = null, version: String? = null): Response = withContext(Dispatchers.IO) {
    ...
    companion object { const val ORIGIN = "https://authentication.unlikeotherai.com" }
}
