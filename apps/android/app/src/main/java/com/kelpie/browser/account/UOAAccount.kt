...
object UOAAccount {
    data class Profile(val subject: String, val email: String, val name: String?)
    data class State(val profile: Profile? = null, val avatar: ByteArray? = null,
        val signingIn: Boolean = false, val error: String? = null)
    private data class Attempt(val id: UUID, val authorization: UOAAuthorization, val clientId: String)
    ...
    fun detach(activity: MainActivity) { if (host.get() === activity) host.clear() }
    private fun openPendingLogin() {
        ...
        if (handler != null && handler.activityInfo.packageName != activity.packageName) {
            try { activity.startActivity(intent); return } catch (_: android.content.ActivityNotFoundException) { }
        }
        fallbackOpen = true
        activity.openAccountFallback(url, generation.toString())
    }
    fun finishFallback(callback: String?, nonce: String?) {
        ...
        scope.launch {
            try {
                val client = run {
                    ...
                    transport.request("/oauth/register", "POST", body = body.toString()).json()["client_id"]!!.jsonPrimitive.content
                }
    ...
    fun receiveCallback(callback: String) {
        val pending = attempt ?: run { mutableState.value = state.value.copy(error = "Login expired. Please try again."); return }
        ...
        scope.launch {
            try {
                val body = buildJsonObject {
                    put("grant_type", "authorization_code"); put("code", code); put("client_id", pending.clientId)
                    put("redirect_uri", UOAAuthorization.CALLBACK); put("code_verifier", pending.authorization.verifier)
                }
                val response = transport.request("/oauth/token", "POST", body = body.toString()).json()
                if (generation != pending.id) return@launch
                val duration = response["expires_in"]!!.jsonPrimitive.long.coerceAtMost(86400).also { require(it > 0) }
                ...
                lifetime = scope.launch { delay((expiresAt - System.currentTimeMillis()).coerceAtLeast(0)); if (generation == pending.id) fail("Your session expired. Please log in again.") }
    ...
    private fun fail(message: String) { signOut(); mutableState.value = State(error = message) }
    fun dismissError() { mutableState.value = state.value.copy(error = null) }

    suspend fun request(path: String, method: String = "GET", body: String? = null, version: String? = null): UOATransport.Response {
