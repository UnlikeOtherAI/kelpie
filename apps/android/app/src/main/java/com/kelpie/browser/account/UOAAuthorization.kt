...
class UOAAuthorization(val state: String = randomValue(), val verifier: String = randomValue()) {
    ...
    fun url(clientId: String): String {
        val fields = linkedMapOf("response_type" to "code", "client_id" to clientId,
            "redirect_uri" to CALLBACK, "scope" to SCOPES, "state" to state,
    ...
    private fun fields(callback: String): List<Pair<String, String>> {
        ...
        require(uri.scheme == "com.unlikeotherai.kelpie" && uri.host == "oauth" && uri.path == "/callback" &&
            uri.userInfo == null && uri.port == -1 && uri.fragment == null) { "Invalid login callback" }
        val fields = uri.rawQuery.orEmpty().split('&').map { item ->
