...
class UOAAccountTests {
    @Test fun pkceMatchesRfc7636AndCallbacksAreExact() {
        ...
        listOf(
            "$base?state=other&code=one",
            "$base?state=state&code=one&code=two",
            "$base?state=state&state=state&code=one",
            "$base?state=state&code=one#fragment",
            "https://oauth/callback?state=state&code=one",
            "$base?state=state&error=denied&code=one",
        ).forEach {
    ...
    @Test fun conflictsRetryTheIntentAndRetainRemoteChanges() =
        kotlinx.coroutines.runBlocking {
            var puts = 0
            var published = emptyList<Bookmark>()
            var error: String? = null
            val sync =
                AccountBookmarks({ published = it }, { _, value -> error = value }, this) { _, method, body, version ->
                    if (method == "PUT") {
                        assertEquals("v2", version)
                        puts++
                        if (puts == 1) {
                            throw com.kelpie.browser.account.UOATransport
                                .Failure(409)
                        }
                        com.kelpie.browser.account.UOATransport
                            .Response(body!!.toByteArray(), "v3")
                    } else {
                        val value = if (puts == 0) "[]" else "[{\"url\":\"https://remote.example\",\"folder\":\"Keep me\"}]"
                        com.kelpie.browser.account.UOATransport
                            .Response("{\"value\":$value}".toByteArray(), "v2")
                    }
                }
    ...
    @Test fun invalidatedResponseCannotPublishAccountData() =
        kotlinx.coroutines.runBlocking {
            val started = kotlinx.coroutines.CompletableDeferred<Unit>()
            val release = kotlinx.coroutines.CompletableDeferred<Unit>()
            var publications = 0
            val sync =
                AccountBookmarks({ publications++ }, { _, _ -> }, this) { _, _, _, _ ->
                    started.complete(Unit)
                    kotlinx.coroutines.withContext(kotlinx.coroutines.NonCancellable) { release.await() }
                    com.kelpie.browser.account.UOATransport
                        .Response("{\"value\":[]}".toByteArray(), "v1")
                }
            sync.enqueue()
            started.await()
            sync.invalidate()
    ...
    @Test fun aLaterSuccessfulSaveCannotHideAnEarlierFailure() =
        kotlinx.coroutines.runBlocking {
            kotlinx.coroutines.supervisorScope {
                var puts = 0
                val sync =
                    AccountBookmarks({}, { _, _ -> }, this) { _, method, body, _ ->
                        if (method == "PUT") {
                            puts++
                            if (puts == 1) {
                                throw com.kelpie.browser.account.UOATransport
