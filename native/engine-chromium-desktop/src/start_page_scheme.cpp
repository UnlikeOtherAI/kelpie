...
class StartPageResourceHandler final : public CefResourceHandler {
 public:
  ...
  void GetResponseHeaders(CefRefPtr<CefResponse> response,
                          ...
                          CefString&) override {
    // CEF wants the type and the encoding separately; see SplitMediaType.
    const start_page::MediaType media = start_page::SplitMediaType(mime_type_);
    response->SetMimeType(std::string(media.type));
    if (!media.charset.empty()) response->SetCharset(std::string(media.charset));
