...
int main() {
  kelpie::CefRenderer renderer; kelpie::HandlerContext context(&renderer); kelpie::BookmarkStore bookmarks; kelpie::HistoryStore history; StubDeviceInfoProvider device_info; MockControl control;
  int width=1280,height=720;
  kelpie::DesktopHandlerRuntime runtime; runtime.handler_context=&context; runtime.browser_control=&control; runtime.bookmark_store=&bookmarks; runtime.history_store=&history; runtime.device_info_provider=&device_info;
  ...
  kelpie::DesktopRouter router; kelpie::BookmarkHandler bookmark_handler(runtime); kelpie::HistoryHandler history_handler(runtime); kelpie::RendererHandler renderer_handler(runtime); kelpie::ViewportHandler viewport_handler(runtime); kelpie::DomHandler dom(runtime); kelpie::EvaluateHandler evaluate(runtime); kelpie::CookieHandler cookies(runtime); kelpie::DialogHandler dialogs(runtime); kelpie::InspectionHandler inspection(runtime); kelpie::ShellHandler shell(runtime);
  bookmark_handler.Register(router); history_handler.Register(router); renderer_handler.Register(router); viewport_handler.Register(router); dom.Register(router); evaluate.Register(router); cookies.Register(router); dialogs.Register(router); inspection.Register(router); shell.Register(router);
