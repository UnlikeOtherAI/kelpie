...
holds), and the running `kelpie.exe` itself. Only a visible window shows the
stage that failed, so these contenders open visibly, small in the top-left
corner. Each must stop at the local control listener stage: its window reads
`Browser startup failed during local control listener: …`, it publishes no
readiness file, `netstat -ano` never shows it `LISTENING` on the port, and the
holder keeps the port (the running browser still answers with its own token).
The runner then closes that window with `WM_CLOSE` and requires the process to
exit with status 1. One more contender is launched hidden against the running
`kelpie.exe`; it must exit promptly with status 1 under the same port and
readiness checks. The CLI phase launches its alias on the run's `--port`, never
the CLI default `8420`.
