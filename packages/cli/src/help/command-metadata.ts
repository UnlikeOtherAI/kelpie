...
export const commandMetadata: Record<string, CommandHelp> = {
  ...
  "highlight show": { purpose: "Highlight an element", when: "You need to draw a viewer's eye to a specific element before interacting with it", explanation: "Draws a colored ring around the element matching the selector. Supports quick appear or draw animations and optional persistence.", related: ["highlight hide", "click", "script run"] },
  ...
  "browser remove": { purpose: "Remove a local macOS browser alias", when: "A saved alias is no longer needed", explanation: "Deletes the alias and saved runtime state from ~/.kelpie. It does not terminate any running app process.", related: ["browser list", "browser register"] },
