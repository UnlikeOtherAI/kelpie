 ...
 * CLI, and every device reject exactly the same strings.
 */
const partitionId = z
  .string()
...
export const browserTools: BrowserToolDef[] = [
  ...
  // Screenshots
  { name: "kelpie_screenshot", description: "Take a screenshot of the device browser. For LLM use, this defaults to viewport/CSS-pixel resolution so coordinates line up with tap more directly and the image uses less context. Request resolution='native' only when you need full renderer detail. For the smallest image, ask for format='jpeg' with a quality and a maxWidth. The result carries the image once, as an image item; the text is metadata only.", method: "screenshot", schema: { device, fullPage: z.boolean().optional().describe("Capture full page"), format: z.enum(["png", "jpeg"]).optional().describe("Image format"), quality: z.number().int().min(1).max(100).optional().describe("JPEG quality 1-100"), maxWidth: z.number().int().min(1).max(16384).optional().describe("Largest image width in pixels; the image is scaled down (never up) to fit. Honoured on Windows and Linux; a device that ignores it returns SCREENSHOT_OPTION_UNSUPPORTED instead of an oversized image."), resolution: screenshotResolution, tabId }, bodyFromArgs: screenshotBody("viewport") },
...
export const cliTools: CliToolDef[] = [
  // Discovery
  { name: "kelpie_discover", description: "Discover devices on the local network via mDNS, falling back to probing this machine's loopback ports when the browse finds nothing (the Windows browser is loopback-only and never announces itself)", method: "discover", kind: "discovery", schema: { timeout: z.number().optional().describe("Discovery timeout in ms (default 3000)") }, bodyFromArgs: filterBody },
