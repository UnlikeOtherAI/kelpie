...
On Windows, the desktop shell provides a Win32 main window, URL bar, native settings dialog, bookmarks/history/network inspector windows, and toast overlay. The tab strip creates, selects, and closes active tabs; `Ctrl+T`, `Ctrl+W`, `Ctrl+Tab`, `Ctrl+Shift+Tab`, and `Ctrl+L` work after the page takes focus. The URL bar preserves an in-progress same-tab edit, synchronizes when the active tab changes, and offers a selected inline history suffix only after a new character is inserted at the end of unselected text. Escape rejects that suffix and Enter accepts it. Bookmarks and history rows navigate on double-click, while the network inspector has Method, Type, and Source filters.
