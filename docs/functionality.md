...
On Windows, the desktop shell under `apps/windows/` now combines an undecorated rounded Win32 frame with macOS-style close/minimize/maximize dots, a native tab strip, URL bar, native settings dialog, bookmarks/history/network inspector windows, native toast overlay, and the shared Chromium desktop runtime. Like macOS, it remembers the window size, on-screen position, and maximized state across launches in the profile's `settings.json`: the saved position is reused only when at least 120x40 px of the window still lands on a connected monitor, otherwise Windows places it, and explicit `--width`/`--height` flags override the remembered size. A failed startup (for example an occupied `--port`) shows its reason in the window; a launch started hidden exits with a failure code instead. Its agent-control server listens only on loopback and requires the per-launch capability from the protected readiness file. Windows screenshots are of the visible viewport, as PNG by default or as JPEG with a `quality`, and `maxWidth` scales the image down to fit a width, so an agent can ask for a 960 px JPEG of a full-size window. Chromium does the encoding and the scaling itself, and the response's `width` and `height` are the returned image's own pixels. Full-page and annotated screenshot requests return an explicit unsupported-parameter error. While the window is minimised or has no visible area, a screenshot returns `WINDOW_MINIMIZED` instead of a stale or tiny image; Linux, which shares the engine, has no such check yet. `wait-for-navigation` waits for the navigation that followed the tab's latest action, including one the page started itself, such as the page a clicked link opens. Console/network logs and viewport size are browser-wide state, so those methods reject `tabId` and `generation` rather than silently using a different tab.
