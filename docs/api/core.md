...
Navigation, DOM access, interaction, scrolling, viewport/device info, wait/sync. Screenshots are in [screenshots.md](screenshots.md).
...
`screenshot` — the viewport or the full page, PNG or JPEG, `maxWidth` on desktop
Chromium, the viewport-mapping metadata and the `WINDOW_MINIMIZED` error — is
described in [screenshots.md](screenshots.md).
...
### `waitForNavigation`
Wait for a navigation to complete, such as the page a clicked link opens.
`navigate` already waits for its own load where its `timeout` is honoured (see
`navigate` above), so this method is for navigations that other actions start.
