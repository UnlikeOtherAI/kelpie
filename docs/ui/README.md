...
- The Windows shell uses an undecorated, softly rounded Win32 frame with a subtle one-pixel border and macOS-style close, minimize, and maximize dots. The custom title area keeps native dragging, edge resizing, double-click maximize, system-menu shortcuts, and normal maximized edges.
- The URL bar uses native `EDIT` and `BUTTON` controls. Its Menu button owns the Bookmarks, History, Network Inspector, and Settings actions that previously lived in the removed native menu bar, and the network inspector mirrors the three desktop/mobile filter groups: Method, Type, and Source.
